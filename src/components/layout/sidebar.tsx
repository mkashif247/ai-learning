    ...
    <aside className="fixed left-0 top-16 bottom-0 w-64 border-r border-slate-800 bg-slate-950 hidden lg:block">
      <nav className="p-4 space-y-1">
        {sidebarItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-5 w-5", isActive ? "text-violet-400" : "text-slate-500")} />
              {item.label}
            </Link>
          );
        })}
      ...
      <div className="absolute bottom-6 left-4 right-4">
        <div className="rounded-xl bg-slate-900 border border-slate-800 p-4">
          <h4 className="text-sm font-medium text-slate-200 mb-1">Upgrade Plan</h4>
          <p className="text-xs text-slate-500 mb-3">Unlock unlimited AI features</p>
